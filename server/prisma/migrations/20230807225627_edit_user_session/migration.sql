/*
  Warnings:

  - The primary key for the `usersession` table will be changed. If it partially fails, the table could be left without primary key constraint.
  - You are about to drop the column `id` on the `usersession` table. All the data in the column will be lost.
  - Added the required column `sessinId` to the `UserSession` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE `usersession` DROP PRIMARY KEY,
    DROP COLUMN `id`,
    ADD COLUMN `sessinId` INTEGER NOT NULL AUTO_INCREMENT,
    ADD PRIMARY KEY (`sessinId`);
